...
| same-second-tasks-sort-by-id-1-6 | Number Every Task A Mutation Writes, And Check Stored Sequences Raw | a mutation appending a task with `Seq` 0 is written and cached with `seq` N+1, never 0 (§2.2, §3.2), with no further write `tick list` and `tick ready --count 1` order the appended task last, not first (§1.2, §8.1), `tick rebuild` and `ReadTasks` return the numbers the mutation cached, so the cache equals a parse of the written bytes (§2.3), unnumbered records lying between numbered ones in a mutated slice are numbered above every carried sequence in record order (§2.1, §2.3), `TestCreateAssignsSequence` reads each record's raw `seq` rather than the read-time backfill, the per-site `NextSeq` calls and the migrate `mockStore` subtests stay unchanged (§8.5) |
