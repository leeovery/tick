...
### Task 1: Add negative-case test confirming terminal siblings excluded from cascade output

**Problem**: The current test suite includes a test (`"it includes unchanged terminal children in cascade output"` in `transition_test.go` line 514) that **asserts** unchanged terminal siblings appear in cascade output. After the fix, there is no test confirming the opposite -- that terminal siblings are excluded. A negative-case test is needed to lock in the desired behavior before the deletion task removes the unchanged feature.

**Solution**: Add an integration test in `internal/cli/transition_test.go` that sets up a parent with one non-terminal child (open) and two terminal siblings (one done, one cancelled), runs a cancel transition on the parent, and asserts the terminal siblings do NOT appear in stdout.

**Outcome**: A new test exists that will pass only when unchanged output is removed. It fails against the current code (confirming it tests the right thing) and will pass after Task 2 completes the deletion.

**Do**:
- In `internal/cli/transition_test.go`, within `TestTransitionCommands`, add a new subtest: `"it excludes terminal siblings from cascade output"`.
- Set up three children under a parent: one open child (will cascade), one done child, one cancelled child.
- The parent is `in_progress`. Cancel the parent via `runTransition(t, dir, "cancel", parentID)`.
- Assert `exitCode == 0`.
...
- [ ] Test compiles and passes after Task 2 is applied (it is expected to fail before Task 2)

**Tests**:
- `"it excludes terminal siblings from cascade output"` -- primary negative-case test

**Edge Cases**:
- Terminal child with `done` status must not appear
- Terminal child with `cancelled` status must not appear
- Both edge cases are covered in the single test by including one child of each terminal status

**Context**:
> The specification states: "Add a test confirming terminal siblings are NOT included in cascade output." The existing test at line 514 of `transition_test.go` (`"it includes unchanged terminal children in cascade output"`) tests the opposite behavior and will be removed in Task 2. This new test replaces it as the canonical assertion for the fixed behavior.
>
> The test uses `runTransition` helper (line 14 of `transition_test.go`) which sets `IsTTY: true`, defaulting to PrettyFormatter. Since PrettyFormatter renders unchanged entries in the tree, the negative assertion (no unchanged IDs in output) covers the pretty format path. The toon format is covered by the formatter-level tests updated in Task 2.

**Spec Reference**: `.workflows/cascade-unchanged-noise/specification/cascade-unchanged-noise/specification.md` -- "Testing" section, bullet "Add a test confirming terminal siblings are NOT included in cascade output"

---

## cascade-unchanged-noise-1-2 | approved

### Task 2: Remove UnchangedEntry type and all unchanged collection and rendering
...
**Solution**: Remove the `UnchangedEntry` type, the `Unchanged` field from `CascadeResult`, the collection logic in `buildCascadeResult()`, the rendering code in all three formatters, and update all tests that reference unchanged behavior. This is a pure deletion -- no new behavior is introduced.

**Outcome**: Cascade output shows only the primary transition and actually-cascaded entries. No "(unchanged)" lines appear in any format. All tests pass, including the negative-case test from Task 1.
...
- `"it excludes terminal siblings from cascade output"` -- Task 1's negative-case test now passes
