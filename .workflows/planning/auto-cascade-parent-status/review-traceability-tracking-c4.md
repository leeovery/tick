...
- Rules 1, 7, 8, 9, 10, 11 (validation/existing) in Phase 1 tasks acps-1-1 through acps-1-6
- Rules 2, 3, 4, 5 (cascade logic) in Phase 2 tasks acps-2-3 through acps-2-6
- Rule 6 (new child to done parent) in Phase 3 tasks acps-3-4 and acps-3-5

Transition history (struct, JSONL field, SQLite table, schema version bump) covered by acps-2-1 and acps-2-2.

CLI display (Pretty tree, Toon flat, JSON structured, FormatCascadeTransition interface method, unchanged terminal children) covered by acps-3-1 and acps-3-2.
...
Reparenting note (Rule 3 re-evaluation on original parent, no cascade reversal) covered in acps-3-5.

ValidateAddChild pure validation with caller-side Rule 6 reopen covered in acps-1-3 (validation), acps-3-4 (create caller), acps-3-5 (update caller).
