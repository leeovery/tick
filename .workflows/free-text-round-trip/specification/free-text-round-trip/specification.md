...
This section scopes to toon and pretty, the formats where these commands print a line of text. Under `--json` all five already return an object today and continue to — `FormatDepChange` returns `{"action","task_id","blocked_by"}`, `FormatMessage` returns `{"message"}`, and `FormatRemoval` returns `{"removed":[…],"deps_updated":[…]}` — so under JSON they are documents, and §11's must-parse coverage reaches them there.
