...
**The one exception is a value pretty was already reporting incorrectly.** The rule preserves pretty's shape, not a wrong number inside it: where this work's conformance passes expose a pre-existing defect in a figure the terminal shares with the agent formats, the figure is corrected in all three rather than kept wrong in one. It has arisen once. `tick dep tree`'s summary derives `chains` and `blocked` from every task in a dependency while deriving `longest` — and the drawn tree — only from tasks no other task blocks, so a project containing a dependency cycle reported `2 chains, longest: 1, 4 blocked` against a tree showing one chain and one blocked task: two halves of one line computed over different sets. `longest` now walks the same set as the counts beside it. Pretty's tree rendering is untouched, since a cycle has no root to draw from.
...
**The counts and the edge list never disagree.** A third state reaches the same emptied edge list without meaning "nothing is blocked": a project where no participant is a root, because every dependency edge sits inside a cycle or every blocker is a dangling ID. The edge list is derived by walking down from roots while the counts are derived from every participant, so that state would otherwise report a count-zero edges section beside a non-zero `chains` and `blocked`. **The full-graph edge list therefore covers every participant, not only those a root reaches**: after the walk from the roots, any participant not yet emitted seeds a further walk. Reading the counts then tells an agent what it got, as above, rather than telling it the two halves of one document contradict each other.
