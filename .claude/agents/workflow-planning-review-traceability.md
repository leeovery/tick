...
6. **Trace, don't invent** — if the plan requires something of the product the spec never decided, flag it; a mechanism the spec does not decide is flagged on the same terms. Don't justify it — the fix is removal.
7. **Spec-grounded fixes** — proposed content comes from the specification; a mechanism it leaves open is never proposed, only removed. Do not hallucinate product content.
