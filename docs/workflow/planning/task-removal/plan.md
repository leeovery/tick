...
| ID | Name | Edge Cases | Status | Ext ID |
|----|------|------------|--------|--------|
| task-removal-3-1 | Bulk argument parsing with deduplication | duplicate IDs silently deduplicated, single ID still works, mixed flags and positional args | pending | |
| task-removal-3-2 | All-or-nothing ID validation for bulk removal | first ID valid but second invalid, all IDs invalid | pending | |
| task-removal-3-3 | Cascade descendant collection | deep hierarchy (3+ levels), task with no children, child removal does not cascade upward | pending | |
