...
// Run executes the orphaned parent check. It reads the tick directory path from
// the context (via TickDirKey), parses task relationships, and checks that every
// non-empty parent reference points to a known task ID. Returns a single passing
// result if no orphans are found, or one failing result per orphaned reference.
func (c *OrphanedParentCheck) Run(ctx context.Context) []CheckResult {
