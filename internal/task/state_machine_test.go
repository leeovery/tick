...
func TestStateMachine_Transition_DoesNotCheckParentStatus(t *testing.T) {
	var sm StateMachine

	t.Run("it allows reopen under cancelled parent when called directly", func(t *testing.T) {
		child := Task{ID: "tick-child", Status: StatusCancelled, Parent: "tick-parent", Closed: closedTime()}

		_, err := sm.Transition(&child, "reopen")
		if err != nil {
			t.Fatalf("Transition should not check parent status, got error: %v", err)
		}
